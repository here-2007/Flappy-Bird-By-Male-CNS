...
from simulation.plasticity import PPL101Plasticity, _scipy_csr_to_torch_sparse
...
def parse_args():
    ...
    p.add_argument("--no-viz",    action="store_true",
                   help="Skip dashboard (faster; for benchmark runs)")
...
def main():
    ...
    video_path = None if args.no_video else CFG.VIDEO_OUT
    ...
    for episode in range(1, args.episodes + 1):
        ...
        while game.state.alive and frame_count < CFG.MAX_FRAMES_PER_EP:
            ...
            # ── Dashboard update ──────────────────────────────────────────
            if dashboard and frame_count % CFG.VIZ_EVERY_N_FRAMES == 0:
                dashboard.update(
                    rgb_frame   = frame_rgb,
    ...
    print(f"[done] Video: {video_path or 'not saved'}")
